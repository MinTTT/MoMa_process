...
def trainGenerator_seg(batch_size,
    ...
    if preload:
        for filename in image_name_arr:
            ...
            preload_mask.append(
                readreshape(os.path.join(mask_path, os.path.basename(filename)), target_size=target_size, binarize=True,
                            order=0, rangescale=False))
            if weight_path is not None:
                preload_weight.append(
                    readreshape(os.path.join(weight_path, os.path.basename(filename)), target_size=target_size,
                                order=0), rangescale=False)
    ...
    while True:
        ...
        for _ in range(batch_size):
            ...
            if preload:
                ...
                mask = readreshape(os.path.join(mask_path, os.path.basename(filename)), target_size=target_size,
                                   binarize=True, order=0, rangescale=False)
                if weight_path is not None:
                    weight = readreshape(os.path.join(weight_path, os.path.basename(filename)), target_size=target_size,
                                         order=0, rangescale=False)
                else:
                    weight = []
    ...
    def generator(files_path, files_list, target_size):
        for index, fname in enumerate(files_list):
            img = readreshape(os.path.join(files_path, fname),
                              ...
                              order=1)
            img = np.reshape(img, (
            1,) + img.shape)  # Tensorflow needs one extra single dimension (so that it is a 4D tensor)
            yield img
...
def trainGenerator_track(batch_size,
    ...
    while True:
        ...
        for _ in range(batch_size):
            ...
            # Read images:
            img = readreshape(filename, target_size=target_size, order=1)
            seg = readreshape(os.path.join(seg_path, os.path.basename(filename)), target_size=target_size,
                              binarize=True, order=0, rangescale=False)
            previmg = readreshape(os.path.join(previmg_path, os.path.basename(filename)), target_size=target_size,
                                  order=1)
            segall = readreshape(os.path.join(segall_path, os.path.basename(filename)), target_size=target_size,
                                 binarize=True, order=0, rangescale=False)

            mother = readreshape(os.path.join(mother_path, os.path.basename(filename)), target_size=target_size,
                                 binarize=True, order=0, rangescale=False)
            daughter = readreshape(os.path.join(daughter_path, os.path.basename(filename)), target_size=target_size,
                                   binarize=True, order=0, rangescale=False)
            ...
            # Data augmentation:
            [img, seg, previmg, segall, mother, daughter, bkgd] = data_augmentation(
                [img, seg, previmg, segall, mother, daughter, bkgd],
...
def predictCompilefromseg_track(img_path, seg_path, files_list=[],
    ...
    for index, item in enumerate(files_list):
        ...
        if fra > 1:
            ...
            prevsegall = readreshape(os.path.join(seg_path, prevframename), target_size=target_size, order=0,
                                     binarize=True, rangescale=False)
