'''
This file contains model_for_colony definitions and loss/metrics functions definitions
...
from tensorflow.keras.layers import Input, Conv2D, MaxPooling2D, Dropout, UpSampling2D, ZeroPadding2D, LeakyReLU, \
...
def pixelwise_weighted_binary_crossentropy(y_true, y_pred):
    ...
    # This is essentially the o nly part that is different from the Keras code:
...
def unet(input_size=(256, 32, 1), final_activation='sigmoid', output_classes=1):
    ...
    Returns
    -------
    model_for_colony : Model
        Defined U-Net model_for_colony (not compiled yet).
...
# Use the following model_for_colony for segmentation:
...
# Use the following model_for_colony for tracking and lineage reconstruction:
def unet_track(input_size=(256, 32, 4), class_weights=(1., 1., 1.)):
    ...
    model.compile(optimizer=Adam(lr=1e-4), loss=class_weighted_categorical_crossentropy(class_weights),
...
# Use the following model_for_colony for segmentation:
def unet_colony(input_size=(512, 512, 1)):
    '''
    Chambers segmentation U-Net.
