...
for pos in range(xp.positions):
    ...
    # Rotation correction:
    firstframe = xp.getframes(positions=pos, channels=0, frames=0, rescale=(
    0, 1))  # Get first frame for rotation estimation (We assume first channel is trans)
    rot_corr = utils.deskew(firstframe)  # Estimate rotation angle
    # Get trans frames:
    transframes = xp.getframes(positions=pos, channels=0, rescale=(0, 1),
                               rotate=rot_corr)  # Now load all trans frames, with rotation correction
    firstframe = np.expand_dims(np.expand_dims(cv2.resize(transframes[0], (512, 512)), axis=0),
                                axis=3)  # using expand_dims to get it into a shape that the chambers id unet accepts
    ...
    # Drift correction:
    drifttemplate = utils.getDriftTemplate(chamberboxes, transframes[
        0])  # This template will be used as reference for drift correction
    driftcorbox = dict(xtl=0,
                       xbr=None,
                       ytl=0,
                       ybr=max(chamberboxes, key=lambda elem: elem['ytl'])['ytl']
                       )  # Box to match template
    transframes, driftvalues = utils.driftcorr(transframes, template=drifttemplate,
                                               box=driftcorbox)  # Run drift corr
    # Load up fluoresence images, apply drift correction and rotation:
    fluoframes = \
    xp.getframes(positions=pos, channels=[x for x in range(1, xp.channels)], squeeze_dimensions=False, rotate=rot_corr)[
        0]  # Not squeezing in case there is only one fluo channel, but squeezing the first position dimension with [0]
    ...
    for m, chamberbox in enumerate(chamberboxes):
        for i in range(transframes.shape[0]):
            seg_inputs.append(cv2.resize(utils.rangescale(utils.cropbox(transframes[i], chamberbox), (0, 1)),
                                         (32, 256)))
    ...
    for m, chamberbox in enumerate(chamberboxes):
        ...
        for i in range(1, transframes.shape[0]):
            singleseg = utils.getSinglecells(
                seg[m * transframes.shape[0] + i - 1])  # Get single cell masks in the 'old' frame
        ...
        lineage, label_stack = utils.updatelineage(seg[m * transframes.shape[0]],
                                                   label_stack)  # Initialize lineage and label stack on first frame
        for i in range(1, transframes.shape[0]):
            ...
            if frame_idxs:
                ...
                lineage, label_stack = utils.updatelineage(seg[m * transframes.shape[0] + i], label_stack, framenb=i,
                                                           lineage=lineage,
                                                           attrib=attrib)  # Because we use uint16, we can only track up to 65535 cells per chamber
        ...
        label_stack_resized = np.empty(
            [transframes.shape[0], chamberbox['ybr'] - chamberbox['ytl'], chamberbox['xbr'] - chamberbox['xtl']],
            dtype=np.uint16)
        for i in range(0, transframes.shape[0]):  # Go over frames
            # Resize from original stack:
            label_stack_resized[i] = cv2.resize(label_stack[i], label_stack_resized.shape[2:0:-1],
                                                interpolation=cv2.INTER_NEAREST)  # The stack needs to be resized to the actual chamber box size
            ...
            contours = \
            cv2.findContours((label_stack_resized[i] > 0).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[
                0]  # Get contours of single cells
            ...
            for c, contour in enumerate(contours):  # Run through cells in frame
                cellnb = label_stack_resized[i, contour[0, 0, 1], contour[
                    0, 0, 0]]  # Get cell number in stack (opencv indexing is opposite of numpy)
                # Length, width, area:
    ...
    savemat(os.path.join(resfolder, ('Position%06d.mat' % (pos + 1))),
            {'res': res,
             ...
             'proc': {'rotation': rot_corr,
                      'chambers': np.array(
                          [[c['xtl'], c['ytl'], c['xbr'] - c['xtl'], c['ybr'] - c['ytl']] for c in chamberboxes],
                          dtype=np.float64),
